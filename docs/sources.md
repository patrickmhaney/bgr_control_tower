...
```bash
duckdb mock_sources.duckdb        # CLI, or:
python scripts/q.py "select * from mock_sources.sage_x3.SORDER limit 5"
```

Everything is seeded (`SEED = 20260905`), so re-running the generator
reproduces the same data exactly. Diagrams of every system are in
...
| `netstock` | **Inferred** | Grain and column semantics are right (item × location, forecast periods, ABC/XYZ, ROP). Exact names are constructed. |
| `pangea` | **Inferred, and the product is a guess** | The whiteboard was cut off at "Pange…". This is modeled as a generic freight/parcel visibility platform. Open question 9. |
...
The local-menu table is named `APLSTD` here, and you should verify that name
against the install — the mechanism (integer enums resolved through a menu
table plus `ATEXTRA` for translations) is right even if the table name isn't.
And `ITMDES1_0` in `SORDERP` is denormalized at order time, so it drifts from
`ITMMASTER` — that's real X3 behavior, not a bug here.
...
Solid lines are reliable keys; dashed lines are the ones that will eat your
sprint. The measured match rates, and three links this map misses, are in
[data_model.md §2](data_model.md#2-cross-system-erd).
...
The `paycom.gl_mapping` line is drawn solid here and it is not one — none of
its 11 accounts exist in the GL. See data_model.md.
...
12. **Pangea event ordering.** ~1.3% of `tracking_event` rows have timestamps
...
Run these against `mock_sources` in `scripts/q.py`, or directly in the file.
...
`generate_mock_sources.py` is self-contained and needs `pandas`
(`pip install -r requirements-dev.txt`). The `SCALE` block near the top
...
through the Snowflake decision. Rewriting the file shows it as modified in git
even when the rows are identical — see [running_it.md](running_it.md#setup).

`schema.sql` is portable DDL generated alongside the data — drop it into
Postgres or SQL Server when you move from exploration to extraction design.
DuckDB will not reproduce source-system *behavior* (concurrency, referential
integrity, CDC), so plan on a real RDBMS for that phase.
